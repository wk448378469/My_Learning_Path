...
def parse_args():
    ...
    parser.add_argument('-c','--conf',default='conf/mosaic.yml',help='path to the conf file')
    return parser.parse_args()

def main(FLAGS):
    # optimization two style session
    style_features_t = losses.get_style_features(FLAGS)
    # model file path
    training_path = os.path.join(FLAGS.model_path,FLAGS.naming)
    # detection path exist
    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            # log loss network structure
            tf.logging.info('Loss network layers:')
            ...
            # run ~ initializer variable
            sess.run([tf.global_variables_initializer(),tf.local_variables_initializer()])
            # restore loss network
            ...
            try:
                while not coord.should_stop():
                    ...
                    if step % 25 == 0:
                        ...
                        writer.flush()
                    if step % 1000 == 0:
                        saver.save(sess,os.path.join(training_path,'style-model.ckpt-done'))
...
if __name__ == '__main__':
    ...
    args = parse_args()
    FLAGS = utils.read_conf_file(args.conf)     # model parametor
    main(FLAGS)
