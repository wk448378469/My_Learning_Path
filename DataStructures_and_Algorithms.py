...
# 循环队列，就是不断根据入队或出队，调整头指针和尾指针的位置，应该是一个链式存储结构。
# 好准备写一个吧

from collections import deque
queue = deque(['haha','hehe','xixi','gaga','youyou'])
queue.append('enen')        #入队列
queue.popleft()             # 出队列
# 这样是不是有点懒....不过deque可以高效实现插入、删除的双向列表，应该既适合队列和栈！
...
# 字符串比较大小是比较从第一个开始比ASCII，不过也没什么卵用
# 字符串也有顺序存储和链式存储......

# BF算法和KMP算法。。。男朋友算法和看毛片算法

# BF就是看子串在主串的位置的算法，从第一个开始看是否匹配，不匹配第二个，以此类推，比较暴力 时间复杂度O(n*n)
# KMP 我先理解理解再说，有点蒙
# 从这里开始就先不敲代码了，毕竟接下来的第一份工作也不是算法工程师，以后还是有可能的。。。
# 还是把学习的重点放在数据挖掘和机器学习比较好。先把这个课程的内容撸完了，掌握下基本概念先。
...
就是树。。。
    ...
    森林：还是不是很多的树，而是树种每个结点而言，其子树的集合即为森林
...
'''
    树的表达方法有很多种：
    1、双亲表达法。。。。。。。。？
        就是有一个变量来记录他的双亲是谁，根结点的指针是-1就好
    2、孩子表达法？、、、、、、、、
        每个结点都有个指针，指针指向子节点。。。。有点浪费
...
# 二叉树！
'''
    n个结点的有限集合，该集合或为空，或有一个根节点和两颗互不相交的、分别称为根节点的左子树和右子树的二叉树组成
    只要没有三个叉就可以。。。mix度 = 2
    格局上面不是人的语言的定义，应该用递归的形式来搞
'''
